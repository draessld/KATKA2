...
execute_generate_dataset="../build/generate_dataset"
...
if [ ! -f "$execute_generate_dataset" ]; then
    echo "Error: Execute '$execute_generate_dataset' not found."
    exit 1
fi
...
for ((i=0; i<3; i++)); do
    ...
    source $execute_generate_dataset $e $a $b $datasets_folder 
done
